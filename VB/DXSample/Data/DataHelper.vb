...
Namespace DXSample.Data
    ...
    Public Class DataHelper
        ...
        Private Shared ReadOnly EmployeeDepartmentImages As List(Of String) = New List(Of String) From {"administration", "inventory", "manufacturing", "quality", "research", "sales"}

        Private Shared imageFemaleSrc As ImageSource

        Private Shared imageMaleSrc As ImageSource

        Private Shared Serializer As XmlSerializer = New XmlSerializer(GetType(DataHelper))
        ...
        Public Shared Function GetEmployees() As List(Of Employee)
            Return CType(Serializer.Deserialize(GetDataStream()), List(Of Employee))
        End Function

        Public Shared ReadOnly Property ImageFemale As ImageSource
            Get
                If imageFemaleSrc Is Nothing Then imageFemaleSrc = ImageSourceHelper.GetImageSource(AssemblyHelper.GetResourceUri(GetType(DataHelper).Assembly, "Images/Person_Female.png"))
        ...
        Public Shared ReadOnly Property ImageMale As ImageSource
            Get
                If imageMaleSrc Is Nothing Then imageMaleSrc = ImageSourceHelper.GetImageSource(AssemblyHelper.GetResourceUri(GetType(DataHelper).Assembly, "Images/Person_Male.png"))
                Return imageMaleSrc
            End Get
        End Property
    End Class
